 ...
 */
export function alignObjectProp(code: string, options: ParserOptions): string {
    ...
    for (let i = 0; i < segs.length; i++) {
        ...
        Object.keys(group)
            .filter((indentLvl) => Number(indentLvl) > indent)
            .forEach((indentLvl) => {
